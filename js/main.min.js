...
var totalSum=0;initialize();function initialize(){var a=new XMLHttpRequest();var b={callType:"init"};a.open("POST","backend/main.php",true);a.setRequestHeader("Content-Type","application/x-www-form-urlencoded; charset=UTF-8");
...
}else{console.log("Oh noesz");}};}function parseMatrix(k,v){var d=Object.keys(v);totalSum=0;for(var s=d.length-1;s>=0;s--){totalSum+=parseInt(v[d[s]]);
}var u=k;var e=Object.keys(u);var q=[];var w;var c;for(var s=0;s<e.length;s++){w=[];c=Object.keys(u[e[s]]);for(var r=0;r<c.length;r++){w[globalLanguageIndexConverter[c[r]]]=parseInt(u[e[s]][c[r]]);
}q.push(w);}var l=d3.layout.chord().padding(0.05).sortSubgroups(d3.descending).matrix(q);var m=1000,f=1100,a=Math.min(m,f)*0.41,n=a*1.1;r1=a;r0=n;var p=d3.scale.category20();
...
});var t=h.selectAll("g.group").data(l.groups).enter().append("svg:g").attr("class","group").on("mouseover",o(0.02)).on("mouseout",o(0.8));t.append("svg:path").style("stroke",function(g){return p(g.index);
}).style("fill",function(g){return p(g.index);}).attr("d",b);t.append("svg:text").each(function(g){g.angle=(g.startAngle+g.endAngle)/2;}).attr("dy",".35em").attr("text-anchor",function(g){return g.angle>Math.PI?"end":null;
...
});function o(g){return function(x,j){h.selectAll(".chord path").filter(function(y){return y.source.index!=j&&y.target.index!=j;}).transition().style("opacity",g);
